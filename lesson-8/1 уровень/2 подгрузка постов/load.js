...
// Определение счётчика постов
//Пришлось объявить переменную на уровне скрипта, 
//т.к. в условии нет, что можно передать в функцию
let  totalCounter = 0; 
...
function insertPosts() {
    ...
    // 3.1 в getPostMarkup в качестве аргумента передавайте счетчик цикла
 	...
 	/* ВОПРОС: не совсем понятно "в качестве аргумента передавайте счетчик цикла"
 	*  тут вроде нет счётчика цикла, мы же исспользуем рекурсию (т.е. счётчик рекурсии)
	*  кроме этоно не понятно зачем передавать данную переменную, т.к.
	*  переменная инициирована должна раньше -это уровень одного поста
	*  в связи с этим сомниваюсь, что понял задачу
 	*/ 
	let postsMarkup = getPostMarkup(totalCounter);
	totalCounter++;
