...
listButtons.forEach(function(elem){
...
function handleClick(clickedButtonEvent) {
    ...
    const card = {
        ...
        img: cardNode.querySelector(".product > img"), // здесь картинка внутри .product
        productName: cardNode.querySelector(".product > .productName"), // здесь .productName, который внутри .product
    ...
    const testOnButton = clickedButtonEvent.target.innerHTML;
    
    if (testOnButton === "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (testOnButton === "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function hideMoreText(card) {
    ...
    let elemDesc = card.wrap.querySelector(".desc");
    elemDesc.parentElement.removeChild(elemDesc);
...
function showMoreText(card) {
    ...
    const text = "Тут будет описание выбранного вами товара";
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из 
    // переменной из п. 6.1
