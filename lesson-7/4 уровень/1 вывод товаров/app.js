...
//Элемет .products
const elemProduct = document.querySelector(".products");

//Добавляю обработчик события на клик
document.querySelectorAll("button").forEach(elem => elem.addEventListener("click",clickHandler));
...
function showCategory(category) {
	...
	products[category].forEach(elem => 
							   elemProduct.innerHTML += getProductMarkup(elem)
							   );
}
...
function getProductMarkup(product) {
	...
	return `
		<div class="product">
			...
			<img src="${product.imageUrl}" alt=""><div>${product.price}</div>
